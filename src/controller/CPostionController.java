...
public class CPostionController {
	...
	private void loadDataFromFile() {
		m_postionList = (PositionList) ProcessFile
				.ReadData(ProcessFile.FILENAME_POSITION);
	...
	private void loadDataOnTable() {
		if (m_postionList == null)
			return;
		m_postionFrame.getTableModel().setRowCount(0);// remove all row in the
														// table
		for (int i = 0; i < m_postionList.size(); i++) {
			Position aPostion = m_postionList.get(i);
			Vector<String> vec = new Vector<String>();
			vec.add(PositionTitle.getTitleString(aPostion.getTitle()));
	...
	public void addEventforAllControl() {
		...
		m_postionFrame.getButtonAdd().addActionListener(
				new CMyProcessButtonEvent());
		m_postionFrame.getButtonUpdate().addActionListener(
				new CMyProcessButtonEvent());
		m_postionFrame.getButtonDelete().addActionListener(
				new CMyProcessButtonEvent());
		m_postionFrame.getComboBoxTitle().addActionListener(
				new CMyProcessButtonEvent());
		m_postionFrame.getButtonSave().addActionListener(
				new CMyProcessButtonEvent());

		m_postionFrame.getButtonExit().addActionListener(
				new CMyProcessButtonEvent());
		m_postionFrame.getDetailTable().addMouseListener(
	...
	private void doAdd() {
		if (m_postionFrame.getButtonAdd().getText().equalsIgnoreCase("Add")) {
			...
			if (m_currentPostion != null) {
				m_postionFrame.getTextFieldSalary().setText(
						m_currentPostion.getSalary() + "");
				m_postionFrame.getTextFieldOtherSalary().setText(
						m_currentPostion.getOtherSalary() + "");
				m_postionFrame.getComboBoxTitle().setSelectedIndex(
						m_currentPostion.getTitle().ordinal());
			}
		}
	...
	private void doUpdate() {
		if (m_postionFrame.getButtonUpdate().getText()
	...
	private void doExit() {
		if (JOptionPane.showConfirmDialog(m_postionFrame,
				"Are you sure you want to close this fuction",
				"Close Position Title", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
	...
	private void doSelectedOnTable() {
		int nRow = m_postionFrame.getDetailTable().getSelectedRow();
		if (nRow >= 0
				&& !m_postionFrame.getButtonAdd().getText()
			...
			m_postionFrame.getTextFieldSalary().setText(
					aPostion.getSalary() + "");
			m_postionFrame.getTextFieldOtherSalary().setText(
					aPostion.getOtherSalary() + "");
			m_postionFrame.getComboBoxTitle().setSelectedIndex(
					aPostion.getTitle().ordinal());
		}
	}
	...
	private void doSave() {
		if (m_postionFrame.getButtonAdd().isEnabled()) {
			...
			if (m_postionList == null)
				m_postionList = new PositionList();
			int nSalary = Integer.parseInt(m_postionFrame.getTextFieldSalary()
					.getText());
			int nOtherSalary = Integer.parseInt(m_postionFrame
					.getTextFieldOtherSalary().getText());

			int nIndex = m_postionFrame.getComboBoxTitle().getSelectedIndex();

			PositionTitle posTitle = PositionTitle.values()[nIndex];
			...
			Vector<String> vec = new Vector<String>();
			vec.add(m_postionFrame.getComboBoxTitle().getSelectedItem()
					.toString());
			...
			boolean bResult = ProcessFile.WriteData(m_postionList,
					ProcessFile.FILENAME_POSITION);
			if (bResult) {
